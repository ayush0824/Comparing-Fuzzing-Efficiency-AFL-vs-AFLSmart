...
int process(char *input)
{
	...
	{ // upper case command
		...
		out = malloc(len + 1);  // Fix: Allocate memory only for the required size
		...
		out[len] = '\0'; // Fix: Add a null terminator at the end of the string
		strncpy(out + len, rest + len, strlen(rest + len) + 1); // Fix: Use strncpy instead of strcat
		printf("%s", out);
		free(out);
	}
